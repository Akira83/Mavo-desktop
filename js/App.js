...
App.loadModels = function(){
	...
	App.graph.on("add", function(cell){
		if (cell instanceof joint.dia.Link){
			...
			//Action if it is added an Actor
			if (cell instanceof joint.shapes.basic.Actor){
				//Send Actor to back
				cell.toBack();
			//Action for Intentional Elements
			}else{
				//Adding atributes to save current values
				cell.attr(".satvalue/values", []);
			}
			
			//Actions for Actors and Intentional Elements
			//Add unique name for each element
			cell.attr(".name/text", cell.attr(".name/text") + "_" + App.graph.getElements().length);
		}	
		//Actions for every element
		cell.attr(".mavo/annotation", []);
		cell.attr(".mavo/set-size", "");
